...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // 1. Get the button safely
    const btn = document.getElementById('specialBtn');

    // 2. Check if button exists before adding the listener
    if (btn) {
        btn.addEventListener('click', function() {
            ...
            if (form) {
                // Clear the form
                form.reset();
                // Show the message using your helper function
                showStatus("Form cleared! Thank you for checking us out.", "green");
            } else {
                console.error("Error: Could not find the form.");
            }
        });
    } else {
        console.error("Error: Could not find 'specialBtn'. Check your HTML ID.");
    }
